...
export default function ProjectCard({ project }: { project: Project }) {
  ...
  const gallery = (project.galleryUrls as string[] | null) ?? [];
  const photoUrl = project.photoUrl ?? gallery[0] ?? null;

  return (
    <Link
      href={`/loyihalar/${project.id}`}
      className="block group h-full"
    >
      <article className="bg-surface-container-lowest rounded-2xl overflow-hidden shadow-[0_12px_32px_-4px_rgba(19,27,46,0.06)] flex flex-col h-full group-hover:shadow-[0_20px_40px_-4px_rgba(19,27,46,0.1)] transition-shadow duration-300">
        {/* ── Image ── */}
        <div className="h-56 relative overflow-hidden bg-surface-container">
          ...
          {/* Gallery count badge */}
          {gallery.length > 1 && (
            <div className="absolute bottom-4 left-4 bg-on-surface/70 backdrop-blur-sm text-white text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1">
        ...
        <div className="p-5 sm:p-8 flex flex-col flex-grow">
          ...
          <p className="text-on-surface-variant leading-relaxed text-base mb-6 flex-grow">
            {project.description ??
              `Uy hududda ta'lim sifatini oshirish va yoshlar bandligini ta'minlash maqsadida moliyalashtirilgan. Hozirda ${project.studentsCount || "ko'plab"} o'quvchilar ta'lim olishmoqda.`}
          </p>
            ...
            {project.fundedAt && (
              <div>
                ...
                <p className="font-medium text-on-surface-variant">
                  {formatDate(project.fundedAt as unknown as string)}
                </p>
              </div>
            )}
          </div>
