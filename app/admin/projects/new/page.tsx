...
export default function NewProject() {
  ...
  useEffect(() => {
    fetch('/api/admin/data?entity=viloyatlar').then((r) => r.json()).then((d) => setViloyatlar(d.viloyatlar || []));
  }, []);
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-bold text-on-surface">Yangi loyiha</h1>
      <form onSubmit={handleSubmit} className="mt-8 max-w-2xl space-y-6">
        ...
        <div>
          ...
          <input required value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))}
            className="w-full rounded-[8px] bg-surface-low px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20" placeholder="Masalan: Najot Ta'lim Hub" />
        </div>
        <div>
          ...
          <input required value={form.ownerName} onChange={(e) => setForm((f) => ({ ...f, ownerName: e.target.value }))}
            className="w-full rounded-[8px] bg-surface-low px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20" placeholder="To'liq ism" />
        </div>
        <div>
          <label className="mb-1.5 block text-sm font-medium text-on-surface-variant">Tavsif</label>
          <textarea value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
            rows={3} className="w-full resize-none rounded-[8px] bg-surface-low px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20" placeholder="Loyiha haqida qisqacha" />
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            <input type="number" step="0.1" value={form.loanAmount} onChange={(e) => setForm((f) => ({ ...f, loanAmount: e.target.value }))}
              className="w-full rounded-[8px] bg-surface-low px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20" placeholder="100" />
          </div>
          <div>
            ...
            <input type="number" value={form.studentsCount} onChange={(e) => setForm((f) => ({ ...f, studentsCount: e.target.value }))}
              className="w-full rounded-[8px] bg-surface-low px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20" placeholder="50" />
          </div>
        </div>
        <div>
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <span className="text-sm text-on-surface">Darhol e&apos;lon qilish</span>
        </label>
